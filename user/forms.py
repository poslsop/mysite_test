...
class LoginForm(forms.Form):
    ...
    def clean(self):#重写clean方法进行规则校验，例如两次密码是否相等，用户名是否错误等，返回cleaned_data
        username_or_email = self.cleaned_data['username_or_email']#通过定义的字段的前端数据
        password = self.cleaned_data['password']

        user = auth.authenticate(username=username_or_email, password=password)#authenticate用户认证，通过认证就返回一个User对象
        if user is None:
...
class ChangePasswordForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):#传入user,为了方便验证旧的密码是否正确
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
